...
namespace Niels.Generates
{
    ...
    /// 桂馬の右上に進む利きを生成します。
    /// </summary>
    public class KnightUpRightMoveGenerator : MoveGenerator
    {
        /// 利きを生成します。
        /// </summary>
        /// <returns></returns>
        protected override IEnumerable<uint> Generate(BoardContext context, Piece piece, int[][] targetIndexs)
        {
            ...
            movedBoard[(int)BoardType.Main] = (((clearBoard[(int)BoardType.Main] >> 10) | ((clearBoard[(int)BoardType.SubBottom] & boards[(int)BoardType.Main].ClearFile[1]) >> 9)) & ~ownBoard[(int)BoardType.Main]);
            movedBoard[(int)BoardType.SubRight] = ((((clearBoard[(int)BoardType.Main] & ~boards[(int)BoardType.Main].ClearFile[1]) >> 2) | ((clearBoard[(int)BoardType.SubBottom] & 0x80) >> 1)) & ~ownBoard[(int)BoardType.SubRight]);
