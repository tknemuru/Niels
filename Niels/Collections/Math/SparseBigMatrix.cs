...
namespace Niels.Collections.Math
{
    public class SparseBigMatrix
    {
            ...
            foreach (string file in this.m_Files)
            {
                using (StreamReader sr = new StreamReader(file, Encoding.GetEncoding("Shift_JIS")))
                {
                    ...
                    while (true)
                    {
                        ...
                        SparseVector<double> vector = new SparseVector<double>(0D, this.m_Width, this.GetVectorDictionary(line));
                        yield return vector;
                    }
                }
            }
        ...
        private Dictionary<int, double> GetVectorDictionary(string csv)
        {
            ...
            {
                vectorDic.Add(StringHelper.ToInt(itemList[i]), StringHelper.ToDouble(itemList[i + 1]));
            }
