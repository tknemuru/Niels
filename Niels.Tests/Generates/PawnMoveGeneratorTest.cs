...
namespace Niels.Tests.Generates
{
    ...
    {
        [TestMethod]
        public void TestUpdatePawnMoveBlack()
        {
            ...
            var actualValue = MoveProvider.GetPawnMoves(context, GenerateTarget.NoPromote);
        ...
        [TestMethod]
        public void TestUpdatePawnMoveWhite()
        {
            var order = new FoolOrder();
            var expectedValue = new List<uint>();
        ...
        [TestMethod]
        public void TestUpdatePawnMoveBlackExistsCapture()
        {
            var order = new FoolOrder();
        ...
        /// </summary>
        [TestMethod]
        public void TestPawnPromoteMove()
        {
            var order = new FoolOrder();

            // メイン盤
            BoardContextForTest context = new BoardContextForTest(Turn.Black);
            context.AddPiece(Piece.Pawn, 73);          
            var expected = new List<uint>();
            expected.Add(MoveForTest.GetMove(Piece.PawnPromoted, 73, 72, Turn.Black, Piece.Empty, Promote.Yes));
            expected = order.MoveOrdering(expected).ToList();

            var actual = MoveProvider.GetPawnMoves(context, GenerateTarget.All);
